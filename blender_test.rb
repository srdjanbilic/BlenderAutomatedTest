...
class BlenderTesting < Minitest::Test
 ...
 def setup
  ...
  assert(@b.div(class: "span6", index: 0).h1.text.include? "Photorealistic Rendering")
  assert(@b.div(class: "span6", index: 0).p(index: 0).text.include? "Blender now features a powerful new unbiased rendering engine called Cycles that offers stunning ultra-realistic rendering.")
  assert(@b.div(class: "span6", index: 0).p(index: 1).text.include? "The built-in Cycles rendering engine offers:")
  assert(@b.div(class: "span6", index: 0).ul.li(index: 0).text.include? "GPU & CPU rendering")
  assert(@b.div(class: "span6", index: 0).ul.li(index: 1).text.include? "Realtime viewport preview")
  assert(@b.div(class: "span6", index: 0).ul.li(index: 2).text.include? "HDR lighting support")
  assert(@b.div(class: "span6", index: 0).ul.li(index: 3).text.include? "Permissive License for linking with external software")
  ...
  assert(@b.div(class: "span6", index: 1).img(alt: 'Rendering in Blender with Cycles').exists?)
   ...
   assert(@b.div(class: "span6", index: 3).h1.text.include? "Fast Modelling")
   assert(@b.div(class: "span6", index: 3).p(index: 0).text.include? "Blender’s comprehensive array of modeling tools make creating, transforming and editing your models a breeze.")
   assert(@b.div(class: "span6", index: 3).p(index: 1).text.include? "Blender’s modeling tools include:")
   assert(@b.div(class: "span6", index: 3).ul.li(index: 0).text.include? "Keyboard shortcuts for a fast workflow")
   assert(@b.div(class: "span6", index: 3).ul.li(index: 1).text.include? "N-Gon support")
   assert(@b.div(class: "span6", index: 3).ul.li(index: 2).text.include? "Edge slide, collapse and dissolve")
   assert(@b.div(class: "span6", index: 3).ul.li(index: 3).text.include? "Grid and Bridge fill")
   assert(@b.div(class: "span6", index: 3).ul.li(index: 4).text.include? "Python scripting for custom tools and addons")
  
  assert(@b.div(class: "span6", index: 4).h1.text.include? "Realistic Materials")
  assert(@b.div(class: "span6", index: 4).p(index: 0).text.include? "With Blender’s new rendering engine the possibilities for materials are endless.")
  assert(@b.div(class: "span6", index: 4).p(index: 1).text.include? "Key features are:")
  assert(@b.div(class: "span6", index: 4).ul.li(index: 0).text.include? "Complete Node Support for full customization")
  assert(@b.div(class: "span6", index: 4).ul.li(index: 1).text.include? "Physically accurate shaders like glass, translucency and SSS")
  assert(@b.div(class: "span6", index: 4).ul.li(index: 2).text.include? "Open Shading Language (OSL) support for coding unique shaders")
  ...
  assert(@b.div(class: "span6", index: 7).h1.text.include? "Fast Rigging")
  assert(@b.div(class: "span6", index: 7).p(index: 0).text.include? "Transforming a model into a poseable character has never been easier!")
  assert(@b.div(class: "span6", index: 7).p(index: 1).text.include? "Blender offers an impressive set of rigging tools including:")
  assert(@b.div(class: "span6", index: 7).ul.li(index: 0).text.include? "Envelope, skeleton and automatic skinning")
  assert(@b.div(class: "span6", index: 7).ul.li(index: 1).text.include? "Easy weight painting")
  assert(@b.div(class: "span6", index: 7).ul.li(index: 2).text.include? "Mirror functionality")
  assert(@b.div(class: "span6", index: 7).ul.li(index: 3).text.include? "Bone layers and colored groups for organization")
  assert(@b.div(class: "span6", index: 7).ul.li(index: 4).text.include? "B-spline interpolated bones")
  
  assert(@b.div(class: "span6", index: 8).h1.text.include? "Animation toolset")
  assert(@b.div(class: "span6", index: 8).p(index: 0).text.include? "Whether it’s simple keyframing or complex walk-cycles, Blender allows artists to turn their still characters into impressive animations.")
  assert(@b.div(class: "span6", index: 8).p(index: 1).text.include? "Blender’s animation feature set offers:")
  assert(@b.div(class: "span6", index: 8).ul.li(index: 0).text.include? "Automated walk-cycles along paths")
  assert(@b.div(class: "span6", index: 8).ul.li(index: 1).text.include? "Character animation pose editor")
  assert(@b.div(class: "span6", index: 8).ul.li(index: 2).text.include? "Non Linear Animation (NLA) for independent movements")
  assert(@b.div(class: "span6", index: 8).ul.li(index: 3).text.include? "IK forward/inverse kinematics for fast poses")
  assert(@b.div(class: "span6", index: 8).ul.li(index: 4).text.include? "Sound synchronization")
  ...
  assert(@b.div(class: "span6", index: 11).h1.text.include? "Sculpting")
  assert(@b.div(class: "span6", index: 11).p(index: 0).text.include? "Experience the joy of sculpting organic subjects using the built-in sculpting feature set of Blender.")
  assert(@b.div(class: "span6", index: 11).p(index: 1).text.include? "Sculpting in Blender includes:")
  assert(@b.div(class: "span6", index: 11).ul.li(index: 0).text.include? "20 different brush types")
  assert(@b.div(class: "span6", index: 11).ul.li(index: 1).text.include? "Multi-res sculpting support")
  assert(@b.div(class: "span6", index: 11).ul.li(index: 2).text.include? "Dynamic Topology sculpting")
  assert(@b.div(class: "span6", index: 11).ul.li(index: 3).text.include? "Mirrored sculpting")

 assert(@b.div(class: "span6", index: 12).h1.text.include? "Fast UV Unwrapping")
 assert(@b.div(class: "span6", index: 12).p(index: 0).text.include? "Easily unwrap your mesh right inside Blender, and use image textures or paint your own directly onto the model.")
 assert(@b.div(class: "span6", index: 12).p(index: 1).text.include? "Blender allows for:")
 assert(@b.div(class: "span6", index: 12).ul.li(index: 0).text.include? "Fast Cube, Cylinder, Sphere and Camera projections")
 assert(@b.div(class: "span6", index: 12).ul.li(index: 1).text.include? "Conformal and Angle Based unwrapping (with edge seams and vertex pinning)")
 assert(@b.div(class: "span6", index: 12).ul.li(index: 2).text.include? "Painting directly onto the mesh")
 assert(@b.div(class: "span6", index: 12).ul.li(index: 3).text.include? "Multiple UV layers")
 assert(@b.div(class: "span6", index: 12).ul.li(index: 4).text.include? "UV layout image exporting")
